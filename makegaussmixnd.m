...
% 
% centers is a M x N matrix.
% stdevs is a M-long vector, or a single value
...
% 
% points is a P x N matrix
% labels is a P-long vector
%  points(i,:) = (points(i,1),..,points(i,N)) is the i-th point and comes from the
%  labels(i)-th mixture model.
%
% Dinoj Surendran (dinoj@cs.uchicago.edu)
%  15 May 2004
%
% Example: [points,labels]=makegaussmixnd(4,[0 0; 0 5; 5 0; 5 5],1,500);
%  generates 2000 points, 
%    500 are from a gaussian distribution centered at (0,0) with stdeviation 1 
%    500 are from a gaussian distribution centered at (0,5) with stdeviation 1
%    500 are from a gaussian distribution centered at (5,0) with stdeviation 1
%    500 are from a gaussian distribution centered at (5,5) with stdeviation 1
%
% Note: It uses the mvnrnd function that appears in MAtlab 6.5 (but
% not 6.0). 
...
for pp = 1 : P                                  % I hope matlab distinguishes between upper and lower case in windows
  means (pp,:) = centers (labels (pp), :);
end
devs  = diag (repmat (stdevs(labels(pp)), 1, N));
points = mvnrnd (means,devs);                     % assumes Matlab
                                                  % 6.5 at least
